...
複数プロジェクトに適用できる解析機能を実装する際の設計原則。

### プロジェクト設定を解析スコープの唯一の入力とする

解析対象ファイル・モジュール解決のルールはプロジェクト自身の設定ファイル（`tsconfig.json`・`package.json` 等）から読み取る。\
パスやパッケージ名をアナライザー側にハードコードしない。

- 解析スコープはプロジェクト設定の `include`/`exclude` に従う
- モジュール解決（エイリアス・workspace シンボリックリンク等）もプロジェクト設定に従う
- 解析対象ソースコードを変更して解析を通す方法は採用しない

### 設定の不完全さはアナライザー側で正規化する

プロジェクトの設定が解析に不十分な場合、**アナライザー側で補完・正規化**する。\
解析対象プロジェクトの設定ファイルを修正することを前提にしない。

例: `include: ["**/*.ts"]` のように拡張子を限定したパターンは、同系統の拡張子（`*.tsx`・`*.mts`）を自動追加して補完する。

### 低レイヤーの解析結果から高レイヤーを自動導出する

ファイル間エッジ（L3）からパッケージ間エッジ（L2）を集約するなど、上位レイヤーの関係は下位レイヤーの集約で自動導出する。\
上位レイヤーのデータを直接操作・手動追加しない。上位レイヤーの欠落は必ず下位レイヤーの欠落が原因であり、根本を修正する。

### 関係が欠落している場合の調査順序

1. 下位レイヤー（ファイル・シンボル）のノードが解析結果に存在するか確認する
2. 存在しない場合、プロジェクト設定の `include`/`exclude` を確認する
3. バンドラーのエントリーポイントなど、import チェーンに乗らないファイルが漏れていないか確認する
