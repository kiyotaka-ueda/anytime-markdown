...
| パネル | 説明 |
| --- | --- |
| **Note** | マルチページのエージェントノート — スクリーンショット・表を Claude Code に共有 |
| **Dashboard** | Trail Viewer Web UI のエントリーポイント |
| **Database** | SQLite およびリモート DB の接続状態・インポート管理 |
| **C4 Model** | C4 アーキテクチャツリー（L1〜L4）とテスト・カバレッジアクション |
| **Memory** | 現在のプロジェクトの Claude Code メモリファイル一覧 |
