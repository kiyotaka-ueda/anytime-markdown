...
拡張機能のインストール時に、Claude Code スキルが `~/.claude/skills/` に自動配置されます。\
アンインストール時にスキルファイルは自動削除されます。

`/anytime-fcmap`

ソースコード解析から Feature-Component Map（`featureMatrix`）を生成・更新できます。
...
| `anytimeTrail.server.port` | `19840` | C4 データサーバーのポート番号 |
| `anytimeTrail.trailServer.port` | `19841` | Trail Viewer サーバーのポート番号 |
